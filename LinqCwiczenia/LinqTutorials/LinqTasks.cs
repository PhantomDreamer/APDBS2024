...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Select(x => new { Nazwisko = x.Ename, Praca = x.Job });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(x => new { Praca = x.Job, LiczbaPracownikow = Emps.Count() }.Praca);
            return result;
        }
        ...
        {
            bool result = Emps.Any(x => x.Job is "Backend programmer");
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            IEnumerable<object> result = (from emp in Emps
                                          ...
                                          .Where(x => x.Ename is "Brak wartości" && x.Job is null && x.HireDate is null);
            return result;
        }
        ...
        {
            IEnumerable<object> result = from dept in Depts
                                         join empdept in (from emp in Emps
                                                          group emp by emp.Deptno into x
                                                          select x) on dept.Deptno equals empdept.Key
                                         select new {
                                             name = dept.Dname, numOfEmployees = empdept.Count()
                                         };
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.GetEmpsWithOneSubs();
            return result;
        }
        ...
        {
            int result = (from x in arr
                          group x by x into temp
                          orderby temp.Count() % 2
                          select temp.Key).Last();
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = from dept in Depts
                                       join emp in Emps on dept.Deptno equals emp.Deptno into deptemp
                                       where deptemp.Count() == 5 || !deptemp.Any()
                                       orderby dept.Dname
                                       select dept;
            //result =
            return result;
        ...
        {
            //Unforunately, I don't believe <Dept> can work here, since we're not trying to work with the departments
            IEnumerable<object> result = (from emp in Emps
                                        where emp.Job.Contains('A')
                                        group emp by emp.Job into x
                                        where x.Count() > 2
                                        orderby x.Count() descending
                                        select new { Praca = x.Key, LiczbaPracownikow = x.Count()});
            //result =
            return result;
            //Nothing is returned, because there's no job with capital A in it
        }
        ...
        {
            //As before, I do not believe that <Dept> is workable in this scenario
            IEnumerable<object> result = (from emp in Emps
                                        from dept in Depts
                                        select new { emp, dept});
            //result =
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static IEnumerable<Emp> GetEmpsWithOneSubs(this IEnumerable<Emp> emps) {
